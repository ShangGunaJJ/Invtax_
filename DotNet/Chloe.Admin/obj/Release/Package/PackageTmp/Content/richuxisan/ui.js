...
//添加图片放大；
EMW.UI.AddZoomImg = function (imgUrl, current, obj_light, data, revNum) {
    var win = EMW.UI.Dialog({
        url: "/Pages/User/UserImg.html",
        title: "",
        width: 740,
        height: 569,
        showHeader: true,
    ...
    win.imgUrl = imgUrl;
    win.current = current || 0;
    win.Con = obj_light;
    win.D = data || {};
    win.revNum = revNum || {};
}
EMW.UI.DocEditor = function (title, url, readonly) {
    ...
    var win = EMW.UI.Dialog({
        ...
        closable: true
        //isDoc: true,
        //Tools: [{ icon: "notification_sms", text: "聊天" },
        //        { icon: "action_lock", text: "锁定" },
        //        { icon: "image_edit", text: "编辑" },
        //        { icon: "content_save", text: "保存" }
        //]
    });
    win.readonly;
}

//选择Talker用户,添加人员最多只能显示三个，且都显示名字；
EMW.UI.AddPerson = function (_this, IsHeader) {
    var win = EMW.UI.Dialog({
        url: "/Pages/User/UserPage.html",
        title: "选择人员",
        width: 800,
        height: 650,
        showHeader: true,
        ...
        OnOK: function (x) {
            if (x && x.length > 0) {
                $(_this).find(".Avatartype1").remove();
                _this.siblings(".userlast").remove();
                _this.addClass('none');
                for (var i = 0; i < x.length; i++) {
                    $("<dl class='Avatartype1 Avatartype2 UserItem'><dt class='avatarname ani'>" + x[i].Name + "、<a href='javascript:;' class='clearbtn'></a></dt></dl>").appendTo(_this).data("user", x[i]);
                }
            } else {
                $(_this).find(".Avatartype1").remove();
                _this.siblings(".userlast").remove();
            ...
            if (x.length > 3) {
                ...
                $("<div class='userlast'>等" + x.length + "人</div>").insertBefore(_this);
            } else {
    ...
    var item = [];
    var users = $(".selectpeople .UserItem");
    if (users) {
        for (var i = 0; i < users.length; i++) {
            item.push($(users[i]).data("user"));
        }
    }
    win.NoteUser = item;
    win.Choice = 1;
...
//选择任务用户,添加人员最多显示5个，并在后面显示总人数。
EMW.UI.AddPeople = function (_this, Img, num) {
    var n = num || 5;
    var win = EMW.UI.Dialog({
        url: "/Pages/User/UserPage.html",
        title: "选择人员",
        ...
        height: 650,
        OnOK: function (x) {
            _this.siblings('.count').remove();
            if (x) {
                _this.siblings(".UserItem").remove();
                for (var i = 0; i < x.length; i++) {
                    $("<dl class='UserItem' title='" + x[i].Name + "'><dt class='avatarimg ani'><img src='" + UserImagePath + x[i].Image + "' height='30' width='30' onerror='this.src=\"" + Img + "\"' /><a href='javascript:;' class='clearbtn'></a></dt></dl>").insertBefore(_this).data("user", x[i]);
                }
            }
            if (_this.siblings(".UserItem").length > n) {
                $("<a href='#' class='count'>" + _this.siblings(".UserItem").length + "</a>").insertBefore(_this);
                _this.siblings(".UserItem").css({ 'display': 'none' });
                for (var j = 2; j < (n + 2) ; j++) {
                    _this.siblings(".UserItem:nth-child(" + j + ")").css({ 'display': 'block' });
                }
            }
        }
    });
    var item = [];
    var users = _this.siblings(".UserItem");
    if (users) {
        for (var i = 0; i < users.length; i++) {
            item.push($(users[i]).data("user"));
        }
